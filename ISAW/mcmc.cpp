...
Protein::Protein(long int n) {
    ...
    sequence_on_lattice.resize(lattice.lattice_side * lattice.lattice_side, 0); //последовательность мономеров
        ...
        if (typeOfUpdate < p_for_local_update) {
            ...
            if (rand_path == 0) {//переставляем начало в конец
                ...
                if (sequence_on_lattice[new_point] == 0) { //проверка, что в узле нет мономеров
                    ...
                    sequence_on_lattice[new_point] = 1; //2 * distribution2(generators3) - 1; //выбор спина
                    ...
                    if (q_rd < p_metropolis) {
                        ...
                        sequence_on_lattice[start_conformation] = 1; //oldspin;
                ...
                if (sequence_on_lattice[new_point] == 0) { //проверка, что в узле нет мономеров
                    ...
                    //теперь можно и подредактировать контакты
                    previous_monomers[temp] = -1;
                    next_monomers[end_conformation] = -1;

                    //std::cout << "profits " << hh <<std::endl;
                    new_E = E - hh;
                    //std::cout << "new E " << new_E << std::endl;
                    if (new_E > 0) {
                    ...
                    if (q_rd < p_metropolis) {
                     ...
                     //удаляем начало
                        ...
                        sequence_on_lattice[end_conformation] = 1; //oldspin;
...
/*        if (i > steps_to_equilibrium) {
        ...
        if (i > steps_to_equilibrium && i % (number_of_monomers * 100) == 0)
        {
            radius();
            calc_bulk();
...
void Protein::write_file(long int i) {
    ...
    filename = "Geometry_ISAW_" + std::to_string(J) + "_" + std::to_string(h) + "_" + std::to_string(number_of_monomers) + ".txt";
    ...
    out_result << "bulk2 err_bulk2 bulk3 err_bulk3 bulk4 err_bulk4" << " steps" << std::endl;
    ...
    filename = "R2_" + std::to_string(J) + "_" + std::to_string(h) + "_" + std::to_string(number_of_monomers) + "_Homopolymer.txt";
...
void Protein::coord_form() {
    ...
    filename = "Example_of_Conformation_N_" + std::to_string(number_of_monomers) + "_J_" + std::to_string(J) + ".txt";
    outresult.open(filename);
...
void Protein::radius_gyration()
{
    ...
    current = start_conformation;
    long double r, r_notdig;
    long double NoDiagnalEleents = 0;
