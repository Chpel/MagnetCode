    ...
    for (long int i = 0; i < lattice_side * lattice_side; i++) {
        ...
        for (int j = 0; j < ndim2(); j++) {
            ...
            if ((y == 0) && (x == (lattice_side - 1))) {
                map_of_contacts_int[ndim2() * i + 4] = lattice_side * (lattice_side - 1);
            }
            if ((y == 0) && (x < (lattice_side - 1))) {
                map_of_contacts_int[ndim2() * i + 4] = lattice_side * (lattice_side - 1) + x + 1;
            }
            if ((y > 0) && (x == (lattice_side - 1))) {
                map_of_contacts_int[ndim2() * i + 4] = (y - 1) * lattice_side;
            }
            if ((y == lattice_side - 1) && (x == 0)) {
                map_of_contacts_int[ndim2() * i + 5] = lattice_side - 1;
            }
            if ((y == lattice_side - 1) && (x > 0)) {
                map_of_contacts_int[ndim2() * i + 5] = x - 1;
            }
            if ((y < lattice_side - 1) && (x == 0)) {
                map_of_contacts_int[ndim2() * i + 5] = i + 2 * lattice_side - 1;
            }
        }
        x_coords[i] = x;
        y_coords[i] = y;
...
void Protein::coord_form() {
    ...
    std::vector <std::vector<int>> steps = { {1,0}, {-1,0}, {0,1}, {0,-1}, {1, 1}, {-1, -1} };
...
void Protein::write_file(long int i) {
    ...
    filename = "Geometry_Ising_Tri_" + std::to_string(J) + "_" + std::to_string(h) + "_" + std::to_string(number_of_monomers) + ".txt";
    ...
    filename = "BC_Tri_" + std::to_string(J) + "_" + std::to_string(h) + "_" + std::to_string(number_of_monomers) + "_" + std::to_string(nSimulation) + ".txt";
    //filename = "Radius_"+std::to_string(J)+"_"+std::to_string(number_of_monomers)+"_CanonicalIsing.txt";
