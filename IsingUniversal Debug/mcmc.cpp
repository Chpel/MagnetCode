...
Protein::Protein(long int n, std::string l) {
    ...
    sequence_on_lattice.resize(int_pow(lattice->lattice_side, lattice->d()), 0); //последовательность мономеров
    next_monomers.resize(int_pow(lattice->lattice_side, lattice->d()), -1); //номер-ссылка на следующий узел
    previous_monomers.resize(int_pow(lattice->lattice_side, lattice->d()), -1); //номер-ссылка на предыдующий узел

    directions.resize(int_pow(lattice->lattice_side, lattice->d()), -1); //направления из {0,1,2,3}

    ordered_coords.resize(int_pow(lattice->lattice_side, lattice->d()), -1); //пока что для последовательной нумерации
